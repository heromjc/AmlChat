...
import com.amlogic.avpipe.VideoCapture.VideoEncodeSync;
...
public class CameraInstance {
    ...
    public boolean open(boolean front) {
        ...
        if ((numCameras > 0) && (mCameraID < 0)) {
            Log.v(TAG, "\n\n ***** Camera Workaround, direction incorrect!\n\n");
            mCameraID = 0;
            mFacing = direction;
        }
    ...
    public void setPreviewBuffers(int bufferSize, int bufferCount, VideoEncodeSync callback) {
        ...
        if (DEBUG) Log.v(TAG, "--- Camera setPreviewBuffers Complete:  " + now());
    }
    ...
    public void setPreviewTarget(SurfaceHolder holder) {
        ...
        if (!mPreviewStarted && (mCamera != null))
            try {
                if (DEBUG) Log.v(TAG, "--- new camera surface : " + holder);
        ...
        if (DEBUG) Log.v(TAG, "--- Camera setPreviewTarget Complete:   " + now());
    }
    ...
    public void configurePreview(int width, int height, int pixelFormat) {
        if (mCamera != null) {
            ...
            if (pixelFormat != format) {
                if (DEBUG) Log.v(TAG, "--- new camera pixelformat : " + pixelFormat);
                p.setPreviewFormat(pixelFormat);
            }
            if ((size.width != width) || (size.height != height)) {
                if (DEBUG) Log.v(TAG, "--- new camera preview size : " + width + "x" + height);
                p.setPreviewSize(width, height);
            }

            mCamera.setParameters(p);

            if (DEBUG) Log.v(TAG, "--- Camera configurePreview Complete:   " + now());
        } else {
    ...
    public void startPreview() {
        if (mCamera != null) {
            if (DEBUG) Log.v(TAG, "--- Camera startPreview:                " + now());
            if (mEncodeSync != null) {
            ...
            if (DEBUG) Log.v(TAG, "--- Camera startPreview Complete:       " + now());
        }
    }
    ...
    private class CameraPreviewCallback implements Camera.PreviewCallback {
        ...
        public void onPreviewFrame(byte[] data, Camera camera) {
            mEncodeSync.onVideoCaptureFrame(data, 0);
            if (mCamera != null) {
